...
@push('cssFirstView')
    ...
    @if(env('APP_ENV')=='local')
        @vite('resources/sources/main/athlete-first-view.scss')
    @else
        @php
            $manifest           = json_decode(file_get_contents(public_path('build/manifest.json')), true);
            $cssFirstView       = $manifest['resources/sources/main/athlete-first-view.scss']['file'];
        @endphp
        <style type="text/css">
...
@section('content')
    ...
    <div class="pagePageBox">
        ...
        <div id="js_buildTocContentMain_element" class="contentBox">
            @include('wallpaper.athlete.test')
        </div>
    </div>
@endsection
@push('modal')
